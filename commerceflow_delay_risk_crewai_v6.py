...
Pipeline:
1. Ingestion              -> SQLite/BigQuery source to silver parquet
...
Run:
    ...
    python commerceflow_delay_risk_crewai.py --source bigquery --bq-table otimizador-cargas.commerce_gold.delay_risk_features --direct
...
class Config:
    ...
    # Source mode
    source: str = "sqlite"  # sqlite or bigquery
    bq_table: str = "otimizador-cargas.commerce_gold.delay_risk_features"
    gcs_bucket: str = ""
    gcs_prefix: str = "commerceflow/ml_runs/latest"
...
def ingest_orders_from_sqlite(_: str = "") -> str:
    """Ingests CommerceFlow training data from SQLite or BigQuery and saves the silver parquet dataset."""
    try:
        ensure_out()

        if CONFIG.source.lower() == "bigquery":
            try:
                from google.cloud import bigquery
            except Exception as exc:
                return f"INGESTION_ERROR: google-cloud-bigquery is not installed or unavailable: {exc}"

            client = bigquery.Client()

            count_sql = f"SELECT COUNT(*) AS n FROM `{CONFIG.bq_table}`"
            total = int(client.query(count_sql).to_dataframe()["n"].iloc[0])

            limit_sql = f" LIMIT {int(CONFIG.max_rows)}" if CONFIG.max_rows and total > CONFIG.max_rows else ""
            query = f"""
            SELECT *
            ...
            out = (
                "INGESTION_SUCCESS\n"
                f"Source: BigQuery\n"
                f"BigQuery table: {CONFIG.bq_table}\n"
                f"Source rows: {total:,}\n"
                f"Ingested rows: {len(df):,}\n"
...
@tool("upload_outputs_to_gcs")
def upload_outputs_to_gcs(_: str = "") -> str:
    """Uploads ml_outputs artifacts to Google Cloud Storage when CONFIG.gcs_bucket is set."""
    try:
        if not CONFIG.gcs_bucket:
            out = "GCS_UPLOAD_SKIPPED: no --gcs-bucket provided."
            append_result("gcs_upload", out)
            return out
        ...
        if not out_dir.exists():
            out = f"GCS_UPLOAD_ERROR: output dir not found: {out_dir}"
            append_result("gcs_upload", out)
            return out
        ...
        for p in out_dir.rglob("*"):
            ...
            blob_name = f"{CONFIG.gcs_prefix.rstrip('/')}/{rel}"
            bucket.blob(blob_name).upload_from_filename(str(p))
            uploaded += 1

        out = (
            "GCS_UPLOAD_SUCCESS\n"
            f"Bucket: gs://{CONFIG.gcs_bucket}\n"
            f"Prefix: {CONFIG.gcs_prefix}\n"
...
def parse_args():
    ...
    p.add_argument("--source", choices=["sqlite", "bigquery"], default=CONFIG.source)
    p.add_argument("--bq-table", default=CONFIG.bq_table)
    p.add_argument("--gcs-bucket", default=os.getenv("GCS_BUCKET", CONFIG.gcs_bucket))
    p.add_argument("--gcs-prefix", default=os.getenv("GCS_PREFIX", CONFIG.gcs_prefix))
...
def apply_args(a):
    CONFIG.db_path=a.db_path; CONFIG.table_name=a.table_name; CONFIG.source=a.source; CONFIG.bq_table=a.bq_table; CONFIG.gcs_bucket=a.gcs_bucket; CONFIG.gcs_prefix=a.gcs_prefix; CONFIG.max_rows=a.max_rows; CONFIG.target_col=a.target; CONFIG.output_dir=a.output_dir; CONFIG.n_jobs=a.n_jobs; CONFIG.enable_xgboost=not a.no_xgboost; CONFIG.enable_lightgbm=not a.no_lightgbm; CONFIG.git_enabled=not a.no_git; CONFIG.git_push=not a.no_git_push; CONFIG.git_track_large_artifacts=a.track_large_artifacts; CONFIG.git_remote_name=a.git_remote; CONFIG.git_branch=a.git_branch; refresh_paths()


def main():
    args=parse_args(); apply_args(args); ensure_out(); save_json(P["run_results"], [])
    logger.info("Starting CommerceFlow AI DS pipeline | source=%s | bq_table=%s | DB=%s | max_rows=%s", CONFIG.source, CONFIG.bq_table, CONFIG.db_path, CONFIG.max_rows)
    if args.direct:
        run_direct()
    else:
